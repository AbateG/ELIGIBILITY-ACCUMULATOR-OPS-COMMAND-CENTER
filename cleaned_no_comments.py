...
def derive_assignment_label(row):
    """Build a human-readable assignment string from queue + assignee."""
...
def build_case_label(row):
    """Human-readable label for the case investigation dropdown."""
...
def run_sla_evaluation():
    """Evaluate open SLAs. Returns error message string or None on success."""
    try:
        evaluate_open_slas()
        return None
...
def load_cases():
    """Load all support cases with SLA tracking data."""
    rows = fetch_all("""
        SELECT
...
def apply_filters(df):
    """Apply all sidebar filters to the cases dataframe."""
    if df.empty:
        return df
    out = df.copy()
